...
public class 剑指Offer31 {
    public static void main(String[] args) {
        Solution solution = new 剑指Offer31().new Solution();
    }
//leetcode submit region begin(Prohibit modification and deletion)
class Solution {
    public boolean validateStackSequences(int[] pushed, int[] popped) {
        // simulate push and try pop element in popped array order
        Stack<Integer> stack = new Stack<>();
        int i = 0;
        for (int x : pushed) {
            stack.push(x);
            // X get from stack
            // true case: pushed = [1,2,3,4,5], popped = [4,5,3,2,1]
            // false case: pushed = [1,2,3,4,5], popped = [4,3,5,1,2]
